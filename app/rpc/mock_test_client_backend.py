...
class MockTestClientBackend(AsyncHTTPClientBackend):
    ...
    async def get(self, url, data, header, auth, timeout) -> Dict:
        future = asyncio.get_event_loop().run_in_executor(
            None,
            functools.partial(
    ...
    async def put(self, url, data, header, auth, timeout) -> Dict:
        future = asyncio.get_event_loop().run_in_executor(
            None,
            functools.partial(
    ...
    async def post(self, url, data, header, auth, timeout) -> Dict:
        future = asyncio.get_event_loop().run_in_executor(
            None,
            functools.partial(
    ...
    async def delete(self, url, data, header, auth, timeout) -> Dict:
        future = asyncio.get_event_loop().run_in_executor(
            None,
            functools.partial(
